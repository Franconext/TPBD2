IF DB_ID('sakila5') IS NOT NULL
BEGIN
    ALTER DATABASE sakila5 SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE sakila5;
END
GO

CREATE DATABASE sakila5;
GO
USE sakila5;
GO
...
CREATE INDEX IX_pelicula_idioma_original_id ON pelicula(id_idioma);
GO
...
BEGIN
    ...
    SELECT id_pelicula, titulo, descripcion FROM inserted;
END;
GO
...
BEGIN
    ...
    UPDATE pt
      ...
      INNER JOIN inserted i ON pt.id_pelicula = i.id_pelicula;
END;
GO
...
BEGIN
    ...
    DELETE pt
      FROM pelicula_texto pt
      INNER JOIN deleted d ON pt.id_pelicula = d.id_pelicula;
END;
GO
...
BEGIN
    ...
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.actor AS t INNER JOIN inserted AS i ON t.id_actor = i.id_actor;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.pais AS t INNER JOIN inserted AS i ON t.id_pais = i.id_pais;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.ciudad AS t INNER JOIN inserted AS i ON t.id_ciudad = i.id_ciudad;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.direccion AS t INNER JOIN inserted AS i ON t.id_direccion = i.id_direccion;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.categoria AS t INNER JOIN inserted AS i ON t.id_categoria = i.id_categoria;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.idioma AS t INNER JOIN inserted AS i ON t.id_idioma = i.id_idioma;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.tienda AS t INNER JOIN inserted AS i ON t.id_tienda = i.id_tienda;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.empleado AS t INNER JOIN inserted AS i ON t.id_empleado = i.id_empleado;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.cliente AS t INNER JOIN inserted AS i ON t.id_cliente = i.id_cliente;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.pelicula AS t INNER JOIN inserted AS i ON t.id_pelicula = i.id_pelicula;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.pelicula_actor AS t INNER JOIN inserted AS i ON t.id_actor = i.id_actor AND t.id_pelicula = i.id_pelicula;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.pelicula_categoria AS t INNER JOIN inserted AS i ON t.id_pelicula = i.id_pelicula AND t.id_categoria = i.id_categoria;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.inventario AS t INNER JOIN inserted AS i ON t.id_inventario = i.id_inventario;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.alquiler AS t INNER JOIN inserted AS i ON t.id_alquiler = i.id_alquiler;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    IF UPDATE(fecha_modificacion) RETURN;
    UPDATE t SET fecha_modificacion = GETDATE() FROM dbo.pago AS t INNER JOIN inserted AS i ON t.id_pago = i.id_pago;
END;
GO
...
FROM cliente AS cl
INNER JOIN direccion AS d ON cl.id_direccion = d.id_direccion
...
SELECT
    ...
    STRING_AGG(CONCAT(a.nombre, ' ', a.apellido), ', ')
        WITHIN GROUP (ORDER BY a.apellido)                    AS actores
FROM pelicula pe
LEFT JOIN pelicula_categoria pc ON pc.id_pelicula = pe.id_pelicula
LEFT JOIN categoria cat ON cat.id_categoria = pc.id_categoria
LEFT JOIN pelicula_actor pa ON pe.id_pelicula = pa.id_pelicula
LEFT JOIN actor a ON pa.id_actor = a.id_actor
GROUP BY pe.id_pelicula, pe.titulo, pe.descripcion, cat.nombre,
         pe.tarifa_alquiler, pe.duracion, pe.clasificacion;
GO
...
FROM empleado AS e
INNER JOIN direccion AS d ON e.id_direccion = d.id_direccion
INNER JOIN ciudad AS c ON d.id_ciudad = c.id_ciudad
INNER JOIN pais AS p ON c.id_pais = p.id_pais;
GO
...
FROM pago AS pg
INNER JOIN alquiler AS al ON pg.id_alquiler = al.id_alquiler
INNER JOIN inventario AS inv ON al.id_inventario = inv.id_inventario
INNER JOIN tienda AS t ON inv.id_tienda = t.id_tienda
INNER JOIN direccion AS d ON t.id_direccion = d.id_direccion
INNER JOIN ciudad AS c ON d.id_ciudad = c.id_ciudad
INNER JOIN pais AS p ON c.id_pais = p.id_pais
INNER JOIN empleado AS g ON t.id_gerente_empleado = g.id_empleado
GROUP BY t.id_tienda, c.ciudad, p.pais, g.nombre, g.apellido;
GO
...
INNER JOIN alquiler AS al ON pg.id_alquiler = al.id_alquiler
INNER JOIN inventario AS inv ON al.id_inventario = inv.id_inventario
INNER JOIN pelicula AS pe ON inv.id_pelicula = pe.id_pelicula
INNER JOIN pelicula_categoria AS pc ON pe.id_pelicula = pc.id_pelicula
...
WITH actor_categoria AS (
    SELECT DISTINCT
        a.id_actor, a.nombre, a.apellido,
        cat.id_categoria, cat.nombre AS nombre_categoria
    FROM actor a
    LEFT JOIN pelicula_actor pa ON a.id_actor = pa.id_actor
    LEFT JOIN pelicula_categoria pc ON pa.id_pelicula = pc.id_pelicula
    LEFT JOIN categoria cat ON pc.id_categoria = cat.id_categoria
),
categoria_peliculas AS (
    SELECT
        ac.id_actor, ac.id_categoria, ac.nombre_categoria,
        (
            SELECT STRING_AGG(pe.titulo, ', ') WITHIN GROUP (ORDER BY pe.titulo)
            FROM pelicula pe
            INNER JOIN pelicula_categoria pc2 ON pe.id_pelicula = pc2.id_pelicula
            INNER JOIN pelicula_actor pa2 ON pe.id_pelicula = pa2.id_pelicula
...
SELECT
    ...
    a.nombre,
    a.apellido,
    STRING_AGG(CONCAT(cp.nombre_categoria, ': ', cp.peliculas), '; ')
        WITHIN GROUP (ORDER BY cp.nombre_categoria) AS info_peliculas
FROM actor a
LEFT JOIN categoria_peliculas cp ON a.id_actor = cp.id_actor
GROUP BY a.id_actor, a.nombre, a.apellido;
GO

CREATE VIEW dbo.clientes_activos
AS
SELECT
    c.id_cliente,
    c.nombre,
...
INNER JOIN dbo.direccion AS d ON c.id_direccion = d.id_direccion
INNER JOIN dbo.ciudad AS ci ON d.id_ciudad = ci.id_ciudad
INNER JOIN dbo.pais AS p ON ci.id_pais = p.id_pais
WHERE c.activo = 1;
GO

CREATE VIEW dbo.alquileres_pendientes
AS
SELECT
    a.id_alquiler,
    a.fecha_alquiler,
...
INNER JOIN dbo.cliente AS c ON a.id_cliente = c.id_cliente
INNER JOIN dbo.inventario AS i ON a.id_inventario = i.id_inventario
INNER JOIN dbo.pelicula AS p ON i.id_pelicula = p.id_pelicula
WHERE a.fecha_devolucion IS NULL;
GO

CREATE VIEW dbo.resumen_peliculas
AS
SELECT
    p.id_pelicula,
    p.titulo,
...
LEFT JOIN dbo.inventario AS i ON p.id_pelicula = i.id_pelicula
LEFT JOIN dbo.alquiler AS a ON i.id_inventario = a.id_inventario
LEFT JOIN dbo.pago AS pg ON a.id_alquiler = pg.id_alquiler
GROUP BY p.id_pelicula, p.titulo, p.clasificacion, p.tarifa_alquiler;
GO

CREATE VIEW dbo.rendimiento_empleados
AS
SELECT
    e.id_empleado,
...
LEFT JOIN dbo.alquiler AS a ON e.id_empleado = a.id_empleado
LEFT JOIN dbo.pago AS p ON a.id_alquiler = p.id_alquiler
GROUP BY e.id_empleado, e.nombre, e.apellido, e.id_tienda;
GO
...
BEGIN
    ...
    FROM pelicula pe
    INNER JOIN inventario inv ON pe.id_pelicula = inv.id_pelicula
    INNER JOIN alquiler al ON inv.id_inventario = al.id_inventario
    WHERE al.fecha_alquiler <= @p_fecha_efectiva AND al.id_cliente = @p_id_cliente;
    ...
    SELECT @v_pagos = ISNULL(SUM(monto), 0) FROM pago WHERE fecha_pago <= @p_fecha_efectiva AND id_cliente = @p_id_cliente;
...
BEGIN
    ...
    SELECT @v_id_cliente = id_cliente FROM alquiler WHERE fecha_devolucion IS NULL AND id_inventario = @p_id_inventario;
    RETURN @v_id_cliente;
END;
GO
GO
...
BEGIN
    ...
    SELECT @v_alquileres = COUNT(*) FROM alquiler WHERE id_inventario = @p_id_inventario;
    IF @v_alquileres = 0 RETURN 1;

    SELECT @v_prestados = COUNT(al.id_alquiler) FROM inventario inv LEFT JOIN alquiler al ON inv.id_inventario = al.id_inventario WHERE inv.id_inventario = @p_id_inventario AND al.fecha_devolucion IS NULL;
...
BEGIN
    ...
    SELECT @id_cliente = a.id_cliente FROM alquiler AS a WHERE a.fecha_devolucion IS NULL AND a.id_inventario = @id_inventario;
    RETURN @id_cliente;
END;
GO
GO
...
BEGIN
    ...
    SELECT @cantidad_alquileres = COUNT(*) FROM alquiler WHERE id_inventario = @id_inventario;
    IF @cantidad_alquileres = 0 RETURN 1;

    SELECT @cantidad_prestados = COUNT(a.id_alquiler) FROM inventario AS i LEFT JOIN alquiler AS a ON i.id_inventario = a.id_inventario WHERE i.id_inventario = @id_inventario AND a.fecha_devolucion IS NULL;
...
BEGIN
    ...
    SELECT @nombre_completo = CONCAT(nombre, ' ', apellido) FROM dbo.cliente WHERE id_cliente = @id_cliente;
...
BEGIN
    ...
    SELECT @stock_disponible = COUNT(*) FROM dbo.inventario AS i WHERE i.id_pelicula = @id_pelicula AND dbo.inventario_en_stock(i.id_inventario) = 1;
...
BEGIN
    ...
    SELECT @total = ISNULL(SUM(p.monto), 0) FROM dbo.pago AS p INNER JOIN dbo.alquiler AS a ON p.id_alquiler = a.id_alquiler INNER JOIN dbo.inventario AS i ON a.id_inventario = i.id_inventario WHERE i.id_pelicula = @id_pelicula;
...
BEGIN
    SET NOCOUNT ON;
    SELECT id_inventario FROM inventario WHERE id_pelicula = @p_id_pelicula AND id_tienda = @p_id_tienda AND dbo.inventario_en_stock(id_inventario) = 1;
    SELECT @p_cantidad_peliculas = COUNT(id_inventario) FROM inventario WHERE id_pelicula = @p_id_pelicula AND id_tienda = @p_id_tienda AND dbo.inventario_en_stock(id_inventario) = 1;
END;
GO
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT id_inventario FROM inventario WHERE id_pelicula = @p_id_pelicula AND id_tienda = @p_id_tienda AND dbo.inventario_en_stock(id_inventario) = 0;
    SELECT @p_cantidad_peliculas = COUNT(id_inventario) FROM inventario WHERE id_pelicula = @p_id_pelicula AND id_tienda = @p_id_tienda AND dbo.inventario_en_stock(id_inventario) = 0;
END;
GO
GO
...
BEGIN
    ...
    SELECT pg.id_cliente FROM pago AS pg WHERE CAST(pg.fecha_pago AS DATE) BETWEEN @inicio_mes_anterior AND @fin_mes_anterior GROUP BY pg.id_cliente HAVING SUM(pg.monto) > @monto_minimo_comprado AND COUNT(pg.id_cliente) > @compras_minimas_mensuales;

    SELECT @cantidad_premiados = COUNT(id_cliente) FROM #tmp_cliente;

    SELECT c.id_cliente, c.id_tienda, c.nombre, c.apellido, c.email, c.id_direccion, c.activo, c.fecha_alta, c.fecha_modificacion FROM #tmp_cliente AS t INNER JOIN cliente AS c ON t.id_cliente = c.id_cliente;
...
BEGIN
    ...
    SELECT p.id_cliente FROM pago AS p WHERE CAST(p.fecha_pago AS DATE) BETWEEN @inicio_mes_anterior AND @fin_mes_anterior GROUP BY p.id_cliente HAVING SUM(p.monto) > @min_monto_compras AND COUNT(p.id_cliente) > @min_compras_mensuales;

    SELECT @cantidad_clientes = COUNT(id_cliente) FROM #clientes_recompensa;

    SELECT c.id_cliente, c.id_tienda, c.nombre, c.apellido, c.email, c.id_direccion, c.activo, c.fecha_alta, c.fecha_modificacion FROM #clientes_recompensa AS cr INNER JOIN cliente AS c ON cr.id_cliente = c.id_cliente;
...
BEGIN
    SET NOCOUNT ON;
    SELECT i.id_inventario FROM inventario AS i WHERE i.id_pelicula = @pelicula_id AND i.id_tienda = @tienda_id AND dbo.esta_en_stock_inventario(i.id_inventario) = 1;
    SELECT @cantidad_peliculas = COUNT(id_inventario) FROM inventario AS i WHERE i.id_pelicula = @pelicula_id AND i.id_tienda = @tienda_id AND dbo.esta_en_stock_inventario(i.id_inventario) = 1;
END;
GO
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT i.id_inventario FROM inventario AS i WHERE i.id_pelicula = @pelicula_id AND i.id_tienda = @tienda_id AND dbo.esta_en_stock_inventario(i.id_inventario) = 0;
    SELECT @cantidad_peliculas = COUNT(id_inventario) FROM inventario AS i WHERE i.id_pelicula = @pelicula_id AND i.id_tienda = @tienda_id AND dbo.esta_en_stock_inventario(i.id_inventario) = 0;
END;
GO
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT p.id_pelicula, p.titulo, p.descripcion, p.clasificacion, p.tarifa_alquiler, p.duracion FROM dbo.pelicula AS p INNER JOIN dbo.pelicula_categoria AS pc ON p.id_pelicula = pc.id_pelicula WHERE pc.id_categoria = @id_categoria ORDER BY p.titulo;
END;
GO
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT a.id_alquiler, p.id_pelicula, p.titulo, a.fecha_alquiler, a.fecha_devolucion, CASE WHEN a.fecha_devolucion IS NULL THEN 'Pendiente' ELSE 'Devuelto' END AS estado, ISNULL(pg.monto, 0) AS monto_pagado FROM dbo.alquiler AS a INNER JOIN dbo.inventario AS i ON a.id_inventario = i.id_inventario INNER JOIN dbo.pelicula AS p ON i.id_pelicula = p.id_pelicula LEFT JOIN dbo.pago AS pg ON a.id_alquiler = pg.id_alquiler WHERE a.id_cliente = @id_cliente ORDER BY a.fecha_alquiler DESC;
END;
GO
GO
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.alquiler WHERE id_alquiler = @id_alquiler) BEGIN ;THROW 50001, 'El alquiler indicado no existe.', 1; END;
    IF EXISTS (SELECT 1 FROM dbo.alquiler WHERE id_alquiler = @id_alquiler AND fecha_devolucion IS NOT NULL) BEGIN ;THROW 50002, 'El alquiler ya posee una devolución registrada.', 1; END;

    UPDATE dbo.alquiler SET fecha_devolucion = ISNULL(@fecha_devolucion, GETDATE()) WHERE id_alquiler = @id_alquiler;
    SELECT id_alquiler, fecha_alquiler, fecha_devolucion, id_cliente, id_inventario FROM dbo.alquiler WHERE id_alquiler = @id_alquiler;
END;
GO
GO
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.inventario WHERE id_inventario = @id_inventario) BEGIN ;THROW 50003, 'El inventario indicado no existe.', 1; END;
    IF dbo.inventario_en_stock(@id_inventario) = 0 BEGIN ;THROW 50004, 'La película seleccionada no está disponible.', 1; END;
    IF NOT EXISTS (SELECT 1 FROM dbo.cliente WHERE id_cliente = @id_cliente AND activo = 1) BEGIN ;THROW 50005, 'El cliente no existe o se encuentra inactivo.', 1; END;

    INSERT INTO dbo.alquiler (fecha_alquiler, id_inventario, id_cliente, fecha_devolucion, id_empleado) VALUES (GETDATE(), @id_inventario, @id_cliente, NULL, @id_empleado);
    SELECT SCOPE_IDENTITY() AS id_alquiler_generado;
END;
GO
GO
SELECT name AS nombre_vista FROM sys.views WHERE name IN ('clientes_activos', 'alquileres_pendientes', 'resumen_peliculas', 'rendimiento_empleados') ORDER BY name;
GO
SELECT name AS nombre_funcion FROM sys.objects WHERE type IN ('FN', 'IF', 'TF') AND name IN ('obtener_nombre_cliente', 'obtener_stock_pelicula', 'total_recaudado_pelicula') ORDER BY name;
GO
SELECT name AS nombre_procedimiento FROM sys.procedures WHERE name IN ('buscar_peliculas_por_categoria', 'listar_alquileres_cliente', 'registrar_devolucion', 'registrar_alquiler') ORDER BY name;
GO